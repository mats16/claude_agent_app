...
```
app/
├── frontend/   # React + Vite + Ant Design v6 → See @app/frontend/CLAUDE.md
├── backend/    # Fastify + WebSocket + Claude Agent SDK → See @app/backend/CLAUDE.md
│   ├── agent/  # Claude Agent SDK config, MCP servers
│   ├── db/     # Drizzle ORM (PostgreSQL)
│   ├── models/ # Domain models (User, Session, etc.)
│   └── routes/ # REST API + WebSocket
└── shared/     # Shared types (@app/shared) → See @app/shared/CLAUDE.md
```
...
> **📖 See [@app/backend/CLAUDE.md](app/backend/CLAUDE.md#4-authentication-pat-first--sp-fallback) for auth details**
...
> **📖 See [@app/backend/CLAUDE.md](app/backend/CLAUDE.md#4-authentication-pat-first--sp-fallback) for encryption details**
...
### Session Model
> **📖 See [@app/backend/CLAUDE.md](app/backend/CLAUDE.md#2-domain-models-factory-pattern) for detailed patterns**
...
// Access utilities
session.id.toString()      // → "session_01h455vb4pex5vsknk084sn02q"
session.id.getSuffix()     // → "01h455vb4pex5vsknk084sn02q"
session.getAppName()       // → "app-01h455vb4pex5vsknk084sn02q"
session.getBranchName()    // → "claude/session_01h455vb4pex5vsknk084sn02q"
...
### User Model
> **📖 See [@app/backend/CLAUDE.md](app/backend/CLAUDE.md#1-request-context-reqctx) for request context details**
...
> **📖 See [@app/backend/CLAUDE.md](app/backend/CLAUDE.md#3-row-level-security-rls) for detailed RLS patterns**
...
### Frontend State
> **📖 See [@app/frontend/CLAUDE.md](app/frontend/CLAUDE.md#context-first-data-fetching) for Context patterns**
...
**Test organization**: See [@app/backend/CLAUDE.md](app/backend/CLAUDE.md#testing) for detailed patterns

- **Unit tests**: Collocated `*.test.ts` files with mocked dependencies
- **Integration tests**: `__tests__/*.integration.test.ts` with real DB/API/Agent SDK

```bash
npm test                 # Run all tests (all packages)
npm run test:coverage    # Coverage report
cd app/backend && npm run test:unit  # Unit tests only
```

## UI/Design
> **📖 See [@app/frontend/CLAUDE.md](app/frontend/CLAUDE.md#theme-system) for theme system details**
