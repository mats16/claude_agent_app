...
npm run db:migrate      # Run migrations manually (also runs on server startup)
...
# Secrets management
databricks secrets create-scope claude-agent                     # Create scope
databricks secrets put-secret claude-agent database-url          # Set DATABASE_URL secret
databricks secrets list-secrets claude-agent                     # List secrets
```
...
resources:
  - name: database-url-secret
    secret:
      scope: claude-agent    # Databricks secret scope
      key: database-url      # Secret key name
      permission: READ
```
...
env:
  - name: DATABASE_URL
    valueFrom: database-url-secret  # References the resource name above
```

The secret is injected as `DATABASE_URL` environment variable at runtime.
...
### Required (Production)
- `DATABASE_URL` - PostgreSQL connection string (required, throws error if not set)
...
`sessions`, `settings`, and `oauth_tokens` tables have RLS enabled. RLS policies are defined in `schema.ts` using Drizzle ORM's `pgPolicy()`. Queries use `withUserContext()` helper to set `app.current_user_id`:
```typescript
await db.execute(sql`SELECT set_config('app.current_user_id', ${userId}, true)`);
```

### Migration Structure
- **Drizzle Kit managed migrations** in `app/backend/db/drizzle/`
- Migrations are auto-generated from `schema.ts` via `npm run db:generate`
- RLS policies defined in `schema.ts` are included in generated migrations
...
npm run db:generate -- --name=add_new_column
# 3. Restart server (migrations applied automatically)
npm run dev
```
...
- Headers parsed:
  ...
  - `X-Forwarded-Preferred-Username`: User display name (required)
  - `x-forwarded-access-token`: OBO access token (required)
  - `X-Request-Id`: Request UUID (optional)

### User Class
User context is extracted via `User.fromHeaders()` from `app/backend/models/User.ts`:
```typescript
class User {
  readonly sub: string;              // User identifier from IdP (X-Forwarded-User)
  readonly email: string;            // User email (X-Forwarded-Email)
  readonly preferredUsername: string; // User display name (X-Forwarded-Preferred-Username)
  readonly name: string;             // Username from email (part before @)
  readonly accessToken: string;      // OBO access token (X-Forwarded-Access-Token)
  readonly local: UserPaths;         // Local paths: $HOME/users/{name}
  readonly remote: UserPaths;        // Remote paths: /Workspace/Users/{email}

  static fromHeaders(headers: HeadersLike): User;  // Factory method
  ensureLocalDirs(): void;  // Creates ~/.claude/skills and ~/.claude/agents
}
```
...
user.ensureLocalDirs();  // Instance method
```

Path access patterns:
- `user.local.claudeConfigDir` → `/home/app/users/{user.name}/.claude`
- `user.remote.claudeConfigDir` → `/Workspace/Users/{user.email}/.claude`
- `user.accessToken` → OBO token for Databricks API calls and MCP servers
...
The agent uses PAT-first authentication with Service Principal fallback:
- **PAT mode**: When user sets a Personal Access Token, it's used for `ANTHROPIC_AUTH_TOKEN`, `DATABRICKS_TOKEN`, and `DATABRICKS_AUTH_TYPE=pat`
- **Fallback (oauth-m2m)**: Uses Service Principal credentials (`DATABRICKS_CLIENT_ID`/`DATABRICKS_CLIENT_SECRET`)

Environment variables set in `processAgentRequest()`:
```typescript
ANTHROPIC_AUTH_TOKEN: userPersonalAccessToken ?? spAccessToken,  // PAT first, SP fallback
...
### API Authentication (Workspace/Repos)
`/api/v1/workspace` and `/api/v1/repos` endpoints use PAT-first authentication:
- `getAccessTokenForUser(userId)` utility in `app/backend/utils/auth.ts`
- Retrieves user's PAT via `getUserPersonalAccessToken(userId)`
- Falls back to Service Principal token via `getAccessToken()` if PAT not available
...
#### Path Structure
- `user.name` extraction: Email `kazuki.matsuda@databricks.com` → `user.name` = `kazuki.matsuda` (@ より前の部分)
- Claude config: `/Workspace/Users/{user.email}/.claude` → Local: `$HOME/users/{user.name}/.claude`
- Working directory: Each session gets unique isolated directory at `$HOME/sessions/{stub}`
...
- Path constants defined in `app/backend/config/index.ts` (`paths.sessionsBase`, `paths.usersBase`)
- `$HOME` is `/home/app` on Databricks Apps, user's home directory on local development
...
**Operations**:
- **Pull (Restore)**: Downloads `/Workspace/Users/{user.email}/.claude` → `$HOME/users/{user.name}/.claude` with overwrite
- **Push (Backup)**: Uploads `$HOME/users/{user.name}/.claude` → `/Workspace/Users/{user.email}/.claude` with `replace: true` (deletes workspace directory first)
...
│   └── User.ts         # User class with fromHeaders() factory
...
└── utils/              # Shared utilities (databricks, headers, encryption)
```

### Key Files
- `app/backend/models/User.ts` - User domain model with `fromHeaders()` factory and `ensureLocalDirs()` method
...
- `app/backend/utils/auth.ts` - PAT-first authentication utility (`getAccessTokenForUser`)
- `app/backend/utils/claudeSettings.ts` - Generates `settings.local.json` with Claude Code hooks for workspace sync
- `app/backend/utils/headers.ts` - Request header extraction (`extractRequestContext`, uses `User.fromHeaders()`)
...
### Database Layer
- `app/backend/db/schema.ts` - Drizzle ORM table definitions (includes RLS policies)
- `app/backend/db/index.ts` - Database client initialization
- `app/backend/db/sessions.ts` - Session queries with RLS support
- `app/backend/db/drizzle/` - Drizzle Kit managed migrations
- `app/backend/db/migrate.ts` - Migration runner (used on server startup)
- `app/backend/env.ts` - Environment loader (imported first in server.ts)

**Note**: Uses drizzle-orm v1.0.0+ which requires `drizzle({ client })` syntax instead of `drizzle(client)`.
